...
namespace XMADownloader.App
{
    internal static class NLogManager
    {
        public static void ReconfigureNLog(Enums.LogLevel logLevel = Enums.LogLevel.Default, bool saveLogs = false)
        {
            ...
            configuration.AddRule(nlogLogLevel, LogLevel.Fatal, consoleTarget, nlogLogLevel != LogLevel.Info ? "*" : "XMADownloader.App.*");
            if(saveLogs)
                configuration.AddRule(nlogLogLevel, LogLevel.Fatal, fileTarget, nlogLogLevel != LogLevel.Info ? "*" : "XMADownloader.App.*");
            //configuration.AddRule(debug ? LogLevel.Debug : LogLevel.Info, LogLevel.Fatal, consoleTarget, debug ? "*" : "XMADownloader.PuppeteerEngine.*");
